...
from utils import calculate_score, save_high_score, get_daily_high_scores, get_all_time_high_scores, has_played_today, clear_daily_high_scores, create_new_video
...
def submit_answer(user_name):
    ...
    if user_answer == correct_answer:
        ...
        score = calculate_score(time_taken, video_file_path)
...
def score(user_name, score):
    daily_high_scores = get_daily_high_scores()  # This function should return today's high scores
    return render_template('score.html', user_name=user_name, score=score, daily_high_scores=daily_high_scores)

@app.route('/high_scores')
def high_scores():
    all_time_high_scores = get_all_time_high_scores()  # Function to get all-time high scores
    daily_high_scores = get_daily_high_scores()  # Function to get today's high scores
...
scheduler.add_job(func=create_new_video, trigger="cron", hour=0)
scheduler.add_job(func=clear_daily_high_scores, trigger="cron", hour=0)
scheduler.start()
